...
Write_IIC_Command(unsigned char IIC_Command)
{
	uint8 ack;
	i2c_master_start();
...
**********************************************/
bool ICACHE_FLASH_ATTR
Write_IIC_Data(unsigned char IIC_Data)
{
	uint8 ack;
	i2c_master_start();
	...
	if (ack) {
		os_printf("addr not ack when tx write data 1\n");
		i2c_master_stop();
		return false;
	}
	...
	if (ack) {
		os_printf("addr not ack when tx write data 2\n");
		i2c_master_stop();
		return false;
	}
	...
	if (ack) {
		os_printf("addr not ack when tx write data 3\n");
		i2c_master_stop();
		return false;
	}
	i2c_master_stop();
...
OLED_WR_Byte(unsigned dat,unsigned cmd)
{
	if(cmd){
		Write_IIC_Data(dat);
	}
